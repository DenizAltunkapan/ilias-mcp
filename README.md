...
Authenticates against ILIAS and keeps the session in memory while the server is running.

### list_courses

Lists enrolled courses with title, URL, and ref ID.

### server_info

Shows non-sensitive runtime settings such as base URL, client ID, language, timeout, and download directory.

### list_calendar_events

Lists calendar events for a day. The seed parameter uses YYYY-MM-DD.

### list_dashboard_news

Lists personal dashboard news from ilPDNewsGUI.

### list_repository_items

Lists child objects below a repository ref ID, for example courses, folders, files, and forums.

### get_repository_item_details

Fetches page-level details for a repository object and detects a download URL when the object is a file.

### get_file_content

Downloads file content by ref ID. PDFs are parsed to text with pypdf.

### crawl_repository

Recursively walks a course or folder and returns a flat tree with path, type, URL, and ref ID.

### download_file

Downloads one ILIAS file object by ref ID.

### download_repository_files

Recursively downloads matching files below a course or folder. By default, it downloads PDFs and keeps the ILIAS folder structure.
